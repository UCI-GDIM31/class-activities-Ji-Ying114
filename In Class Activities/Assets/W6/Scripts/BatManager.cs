...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //
        foreach (BatW6 b in _bats){
            if (Vector3.Distance(b.transform.position, _playerTransform.position) <= _interactDistance){
                b.chase(_playerTransform.position, speed);
            }

            if (Vector3.Distance(b.transform.position, _playerTransform.position) <= _overlapDistance){
                CreateReactions(b);
            }
        }
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        //      of CreateReactions.
        int r = Random.Range(0, _messages.Length);
        SpawnReactionUI(bat, _messages[r]);
        Debug.Log(r);
        // STEP 5 -------------------------------------------------------------
    }
